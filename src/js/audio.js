...
const Score = (() => {
  ...
  function prepare(onProgress) {
    ...
    preparing = (async () => {
      ...
      const out = off.createGain(); out.gain.value = 0.62;   // pre-shaper: the curve saturates before the shaper's input clamp
      const clip = off.createWaveShaper();
      const curve = new Float32Array(2048);
      for (let i = 0; i < 2048; i++) { const x = i / 1023.5 - 1; curve[i] = 0.97 * Math.tanh(x * 2.5); }
      clip.curve = curve;
